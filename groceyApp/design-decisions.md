...
### Centralized Icon Mapping Utility — 2024-11-14
- **Change**: Created `IconMapper.kt` utility with bidirectional conversion functions (`mapIconToString()` and `mapStringToIcon()`) covering 36 Material Icons. This replaced duplicated icon mapping logic that existed in multiple components (MainActivity, category creation, list creation).
- **User Problem / Goal**: Maintain consistent icon persistence across lists and categories. Previously, icon mapping was duplicated in multiple places, leading to potential inconsistencies and maintenance overhead.
- **Justification**: DRY (Don't Repeat Yourself) principle reduces code duplication and bug surface. Single source of truth for icon conversions ensures all features (lists, categories, products) use identical mapping logic. Supports Nielsen's consistency heuristic by guaranteeing icons render identically across features.
- **Impact**: Icon changes now propagate correctly when editing lists or categories. Centralized mapping simplifies adding new icons—only one file needs updating. Eliminated bugs where icons wouldn't persist after editing because different components used different string representations.

### Product Options Menu with Context-Aware Actions — 2024-11-14
- **Change**: Created `ProductOptionsMenu` component displayed when users tap the three-dot menu on products within category detail views. Menu includes: Move to Category (folder icon), Add to List (+icon), Add to Pantry (kitchen icon), and Delete (trash icon, red). First three options use neutral gray colors; delete uses error red.
- **User Problem / Goal**: Users need to manage products directly from category views—moving them between categories, adding to shopping lists, stocking pantry, or removing unwanted items. Previously, products in category views had no action menu.
- **Justification**: Nielsen's "Flexibility and efficiency of use"—power users can perform advanced actions without leaving context. Consistent three-dot menu pattern (already used for lists/categories) leverages learned behavior. Color coding (gray for actions, red for destructive) follows Material Design error signaling and prevents accidental deletions. Icon choices (folder for move, + for add) follow platform conventions for recognition over recall.
- **Impact**: Users can reorganize their product catalog efficiently. Move to Category enables fixing miscategorized items. Add to List bridges the gap between product catalog and active shopping. Color-coded delete button reduces accidental deletion errors. Menu positioning (opens below button using DpOffset) prevents occlusion and follows thumb-zone accessibility.

### Select Category Dialog for Product Movement — 2024-11-14
- **Change**: Implemented `SelectCategoryDialog` that displays all available categories (except current) with icons and names. Clicking a category moves the product immediately. Dialog shows product name in title ("Move 'ProductName' to:") for context. Categories display their custom icons using the centralized IconMapper.
- **User Problem / Goal**: Users who misclassified products during creation need a quick way to move them to correct categories. Must see all available destinations and confirm which product is being moved to prevent errors.
- **Justification**: Progressive disclosure—dialog appears only when needed, showing focused choice set. Nielsen's "Error prevention"—displaying product name prevents moving wrong items. Showing category icons leverages visual memory (recognition over recall). Filtering out current category prevents no-op moves. Single-tap selection follows Fitts's Law for efficiency (no additional confirmation needed since move is reversible via undo or another move).
- **Impact**: Product recategorization takes 2 taps (menu → category) instead of requiring delete/recreate workflow. Visual category representation helps users with many categories. Contextual dialog title reduces errors. Implementation includes proper backend integration—updates persist to database via ProductViewModel.

### Comprehensive User Feedback via Snackbars — 2024-11-14
- **Change**: Added success/error snackbars for all major CRUD operations: Create List (green), Edit List (green), Delete List (red), Create Category (green), Edit Category (green), Delete Category (red), Create Product (green), Move Product (green), Delete Product (red). Each snackbar includes the affected item's name for specificity.
- **User Problem / Goal**: Users need immediate confirmation that their actions succeeded. Without feedback, users might repeat actions (creating duplicates) or feel uncertain about system state. Error conditions also need clear communication.
- **Justification**: Nielsen's "Visibility of system status"—users should always know what the system is doing. Color coding (green=success, red=deletion) provides instant visual categorization. Including item names reduces ambiguity ("List created: Weekly Groceries" vs generic "List created"). Material Design Snackbar pattern provides non-intrusive confirmation that auto-dismisses. Consistent implementation across all operations builds predictable mental model.
- **Impact**: Users receive confirmation for every action, building trust in the system. Color coding helps users quickly scan feedback without reading text. Named confirmations prevent "did that work?" uncertainty. Red deletion snackbars provide extra assurance that destructive actions completed (important for user control and freedom). Snackbars don't block workflow—users can continue working while feedback displays.

### Product Card Dual-Mode Pattern — 2024-11-14
- **Change**: Enhanced `ProductItemCard` with `ProductCardMode` enum (LIST vs CATEGORY). LIST mode shows checkbox, quantity controls, and category label (existing behavior). CATEGORY mode removes checkbox/quantity, hides category label (redundant in category view), and displays three-dot menu for product actions.
- **User Problem / Goal**: Products appear in two contexts: within shopping lists (where quantity/completion matters) and within category views (where organization matters). Each context needs different affordances without duplicating component code.
- **Justification**: Single Responsibility Principle adapted to context—one component handles both use cases via mode parameter, avoiding divergent implementations. Gestalt figure-ground principle—removing redundant category label in category view reduces visual noise (users already know which category they're viewing). Nielsen's "Aesthetic and minimalist design"—show only relevant controls per context. Mode-based rendering maintains consistent card shell while adapting content.
- **Impact**: Category views feel cleaner without redundant labels and inappropriate controls. LIST mode preserves familiar shopping workflow. Single component reduces maintenance burden—bug fixes benefit both contexts. Mode parameter documents intent explicitly in code. Future modes (PANTRY, SEARCH_RESULT) can extend pattern without component proliferation.

### Category Detail Screen with Product Management — 2024-11-14
- **Change**: Created `CategoryDetailScreen` following same pattern as ListDetailScreen: top app bar with back button and category options menu, scrollable list of products in CATEGORY mode, FAB for adding products, bottom navigation bar. Products display as individual cards with three-dot menus. Screen receives full product/category data from MainActivity and includes dialogs for moving products and confirming deletions.
- **User Problem / Goal**: Users need to view all products in a category, move misclassified items to other categories, or delete obsolete products. Clicking a category card should navigate to focused detail view (parallel to list detail behavior for consistency).
- **Justification**: Consistency heuristic—parallel structure to ListDetailScreen creates predictable navigation pattern. Users learn once, apply to both lists and categories. Material master-detail pattern appropriate for collection-item relationships. Dedicated screen provides space for future features (category-level filters, bulk operations, product sorting). Maintaining bottom nav prevents navigation dead ends (user control and freedom). Three-dot menu reuse leverages learned interaction pattern.
- **Impact**: Categories and lists now have feature parity—both navigate to detail views with appropriate management controls. Users can reorganize their entire product catalog without deleting/recreating items. Consistent navigation reduces learning curve. Parallel implementation simplifies codebase maintenance—patterns established for lists transfer directly to categories.

### Reusable Menu Components for Consistency — 2024-11-14
- **Change**: Refactored category three-dot menus to use shared `CategoryOptionsMenu` component in both category cards and detail screens. Eliminated code duplication between main category list and detail view menus. Component includes built-in Miscellaneous category protection (doesn't render menu for system category).
- **User Problem / Goal**: Category management actions (edit, delete) appear in two locations (main list and detail screen). Duplicated menu implementations could diverge, creating inconsistent behavior. Special handling needed for Miscellaneous category to prevent deletion.
- **Justification**: DRY principle eliminates duplicate menu code. Consistent behavior across contexts prevents user confusion (Nielsen's consistency heuristic). Encapsulating Miscellaneous protection in component prevents implementation errors where protection might be forgotten. Component approach follows Material Design's composition patterns for reusable UI elements.
- **Impact**: Edit and Delete options behave identically whether accessed from main list or detail view. Miscellaneous category automatically gets proper protection in all contexts. Future menu changes (new actions, visual updates) propagate to both locations automatically. Reduced code volume lowers maintenance burden and bug surface.

### Backend Integration Pattern for Product Operations — 2024-11-14
- **Change**: Product move and delete operations correctly integrate with backend API via ProductViewModel. Move to category sends ProductUpdate with both name and categoryId (backend requires name field even for partial updates). Delete operation properly propagates through repository layer with success callbacks triggering UI refresh and user feedback.
- **User Problem / Goal**: Product operations must persist to database, not just update local UI. Backend API validation requirements (mandatory name field) must be satisfied. Users need to see updated data after operations complete.
- **Justification**: Single source of truth principle—backend database is authoritative state; UI reflects that state. Proper error handling via ViewModel ensures API failures surface to users. Including product name in update request satisfies backend validation (discovered via debug logs showing "Missing name" 400 error). Success callbacks chain operations correctly: API call → data refresh → UI update → user feedback.
- **Impact**: Product moves and deletions persist correctly across app restarts and devices. Error conditions (network failures, validation errors) surface to users via error snackbars. Loading products after operations ensures UI reflects latest backend state (important for multi-device scenarios). Debug logging pattern established for future backend integration work.

### Context-Aware Product Creation within Categories — 2024-11-14
- **Change**: Implemented `AddProductToCategoryDialog` that only requests product name (not category selection) when adding products from category detail view. This differs from the web application's generic "Add Product" flow, which always requires category selection. FAB button in category detail screen triggers this simplified dialog. Created product automatically belongs to the current category but can be moved later via the product options menu.
- **User Problem / Goal**: Users viewing a category detail screen want to add products to that specific category. Forcing them to select the category again (when context already implies it) creates unnecessary friction and cognitive load. However, users still need flexibility to move products if they change their mind.
- **Justification**: Principle of least effort—reducing required inputs from two (name + category) to one (name only) when category is contextually obvious. Gulf of Execution (Norman)—system should minimize distance between user intention and required action. When user is viewing "Dairy" category and taps +, their intent is clearly "add to Dairy," not "add to some category to be determined." Context-aware UI adapts to user's current focus, following Nielsen's "Flexibility and efficiency of use." Product mobility via move function preserves user control (Nielsen's "User control and freedom")—if user reconsiders category assignment, they can correct it without deleting/recreating.
- **Impact**: Category-specific product creation is faster and more intuitive than web version. Users don't need to search for or remember which category they're currently viewing. Reduced form complexity lowers barriers to capturing products quickly. Dialog title "Add product to [Category Name]" provides explicit confirmation of destination, preventing errors. Move-to-category option ensures users aren't locked into initial decision. Pattern establishes precedent for context-aware creation in other areas (e.g., pantry items created from pantry screen automatically belong to pantry).

### Pantry Section Implementation — 2024-11-16
- **Change**: Implemented full pantry management system mirroring the products section structure. FAB in Pantry screen directly opens "Add Pantry" dialog (name + icon selection). Pantries display as cards with three-dot menus for edit and delete. Clicking a pantry card navigates to detail screen showing all items with quantity controls. Items display product name, category as subtitle, and a counter with +/- buttons. Item menu allows moving to another pantry or deletion. Adding items to pantry requires product name, category selection, and quantity input.
- **User Problem / Goal**: Users need to track what items they have in stock at home (pantry inventory) to avoid buying duplicates and know when to restock. This differs from shopping lists (what to buy) and product categories (available products). Pantry represents physical inventory with quantities that decrease as items are consumed.
- **Justification**: Code reuse principle—pantries reuse CategoryCard UI component and follow same navigation patterns as Products/Lists sections, reducing learning curve (Nielsen's consistency heuristic). Quantity tracking essential for inventory management—users need to know "I have 3 cans of soup" not just "I have soup." Three-dialog approach (create pantry, edit pantry, add item) provides clear separation of concerns. Move-to-pantry feature supports scenarios where users realize items belong in different storage locations (fridge vs dry pantry). Context-aware product creation (quantity required) aligns with pantry's inventory purpose. Counter UI (vs checkbox) signals continuous quantity rather than binary state.
- **Impact**: Users can now manage household inventory separate from shopping needs. Quantity counters provide at-a-glance stock levels for quick reference while meal planning or shopping. Reusing established UI patterns (cards, FABs, three-dot menus) means users leverage existing knowledge—no new interaction model to learn. Multi-pantry support accommodates real-world scenarios (main pantry, freezer, garage storage). Integration with product database ensures pantry items link to categories, enabling future features like "add low-stock items to shopping list." Parallel implementation with Products section establishes consistent data model for future enhancements (pantry-to-list transfers, expiration tracking).
