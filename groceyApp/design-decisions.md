...
### Auth Connectivity Hardening — 2025-11-16
- **Change**: After auditing `TokenStorage`, `AuthViewModel`, and the Retrofit stack, the `ApiClient` now derives its base URL from `BuildConfig.API_BASE_URL` with safe emulator/physical fallbacks so every device hits the same backend instead of a per-device localhost.
- **User Problem / Goal**: People could only log in with the most recently created account on a given device because each build targeted a different local API. Users expect their validated credentials to work everywhere.
- **Justification**: Nielsen’s “Consistency and standards” heuristic and ISO 9241 guidance on platform parity both require data endpoints to behave identically across contexts. Centralizing the base URL selection also supports the UX mandate to “recognize rather than recall” because users no longer have to remember which device owns which account.
- **Impact**: Accounts are no longer device-scoped; any verified account becomes accessible on any handset/emulator, reducing failed sign-in attempts and restoring trust in the auth system.

### Password Change Flow Wiring — 2025-11-16
- **Change**: Reused the existing `PasswordChangeDialog` component (instead of adding a new surface) and wired it to `AuthViewModel.changePassword` with dedicated state, inline validation, and progress/error feedback surfaced through the shared snackbar.
- **User Problem / Goal**: Users need to update credentials from the profile drawer with immediate confirmation or remediation guidance when something goes wrong.
