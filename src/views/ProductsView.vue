<script setup lang="ts">
  ...
  type Category = { id: number; title: string; icon: string; products: Product[] };
  ...
  const filteredCategories = computed<Category[]>(() => {
    ...
    return categories.value
      .map((cat: Category) => ({
        ...cat,
        products: cat.products.filter((p: Product) => p.name.toLowerCase().includes(q)),
      }))
      .filter((cat: Category) => cat.title.toLowerCase().includes(q) || cat.products.length > 0);
  });

  function sortCategories(list: Category[]): Category[] {
    if (sortBy.value === 'Name') {
      return [...list].sort((a: Category, b: Category) => a.title.localeCompare(b.title));
    }
    if (sortBy.value === 'Items') {
      return [...list].sort((a: Category, b: Category) => b.products.length - a.products.length);
    }
    return list;
  }
  ...
  onMounted(async () => {
    ...
    categories.value = [
      { id: 1, title: 'Fruits', icon: 'mdi-food-apple', products: [
        { id: 101, name: 'Apples' },
        { id: 102, name: 'Bananas' },
        { id: 103, name: 'Strawberries' },
      ]},
      { id: 2, title: 'Vegetables', icon: 'mdi-food-variant', products: [
        { id: 201, name: 'Tomatoes' },
        { id: 202, name: 'Lettuce' },
      ]},
      { id: 3, title: 'Dairy', icon: 'mdi-cheese', products: [
        { id: 301, name: 'Milk' },
        { id: 302, name: 'Cheese' },
        { id: 303, name: 'Yogurt' },
        { id: 304, name: 'Butter' },
...
<template>
  <div class="pa-3">
    <v-container fluid>
      ...
      <!-- No results message -->
      <div v-if="!isLoading && filteredCategories.length === 0 && searchQuery" class="text-center pa-8">
        <v-icon icon="mdi-magnify" size="64" color="grey-lighten-1" />
        <p class="mt-2 text-h6 text-medium-emphasis">No categories or products found</p>
